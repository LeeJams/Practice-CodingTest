...
function solution(words) {
  ...
  words.forEach((word) => {
    ...
    for (let i = 0; i < word.length; i++) {
      wordCombi += word[i];
      const filterWord = words.filter((n) => n.startsWith(wordCombi));
      result++;
  ...
  insert(string) {
    ...
    for (const char of string) {
      if (!currentNode.children.has(char)) {
        currentNode.children.set(char, [new Node(currentNode.value + char), 1]);
      } else {
        const children = currentNode.children.get(char);
        currentNode.children.set(char, [children[0], children[1] + 1]);
      }
      currentNode = currentNode.children.get(char)[0];
    }
  }
  has(string) {
    let currentNode = this.root;
    ...
    for (const char of string) {
      count++;
      if (currentNode.children.get(char)[1] === 1) {
        return count;
      }
      currentNode = currentNode.children.get(char)[0];
    }
