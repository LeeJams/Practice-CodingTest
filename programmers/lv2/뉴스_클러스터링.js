...
function solution(str1, str2) {
  const upStr1 = str1.toUpperCase();
  const upStr2 = str2.toUpperCase();

  const obj = {};

  const engRegExp = /^[A-Z]*$/;

  for (let i = 1; i < upStr1.length; i++) {
    const str = `${upStr1[i - 1]}${upStr1[i]}`;
    if (engRegExp.test(str)) {
      obj[str] = { arr1: (obj[str]?.arr1 || 0) + 1, arr2: obj[str]?.arr2 || 0 };
    }
  }
  for (let i = 1; i < upStr2.length; i++) {
    const str = `${upStr2[i - 1]}${upStr2[i]}`;
    if (engRegExp.test(str)) {
      obj[str] = { arr1: obj[str]?.arr1 || 0, arr2: (obj[str]?.arr2 || 0) + 1 };
    }
  }
  ...
  let union = 0;
  for (let key in obj) {
    if (obj[key].arr1 && obj[key].arr2) {
      union += Math.max(obj[key].arr1, obj[key].arr2);
      intersect += Math.min(obj[key].arr1, obj[key].arr2);
    } else {
      union += Math.max(obj[key].arr1, obj[key].arr2);
    }
  }
